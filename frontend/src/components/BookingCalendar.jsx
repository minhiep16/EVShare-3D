...
const BookingCalendar = ({ bookings, onSelectAll }) => {
  ...
  // Filter bookings that have a purpose and are upcoming (e.g. June 13 and June 16)
  const upcomingBookings = bookings
    .filter(b => b.purpose && (new Date(b.startTime).getDate() >= 13))
    .sort((a, b) => new Date(a.startTime) - new Date(b.startTime));

  return (
    <div className="xl:col-span-6 bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
      <div className="flex items-center justify-between mb-5">
        <h3 className="text-base font-semibold">Lịch đặt xe tháng 6</h3>
        <button 
          onClick={onSelectAll}
      ...
      <div className="mt-6 pt-5 border-t border-slate-100">
        ...
        <div className="space-y-2">
          ...
          {upcomingBookings.length === 0 && (
            <p className="text-sm text-slate-400 text-center py-4">Không có lịch đặt sắp tới</p>
          )}
        </div>
      </div>
    </div>
